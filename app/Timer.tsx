...
type TimerState={mode:Mode;total:number;running:boolean;startedAt:number;elapsedBeforeStart:number;progressSessionId?:string};

const TIMER_KEY="study-x-timer";
const PROGRESS_KEY="study-x-progress";
const PROGRESS_CURSOR_KEY="study-x-progress-cursor";
const PROGRESS_LEADER_KEY="study-x-progress-leader";
...
function newProgressSessionId(){return Date.now().toString(36)+"-"+Math.random().toString(36).slice(2)}
function defaultTimer():TimerState{return{mode:"Timer",total:DEFAULT_TOTAL,running:false,startedAt:0,elapsedBeforeStart:0,progressSessionId:newProgressSessionId()}}
function readTimer():TimerState|null{try{const raw=localStorage.getItem(TIMER_KEY);if(!raw)return null;const s=JSON.parse(raw);if(!s||!modes.includes(s.mode)||!Number.isFinite(Number(s.total))||Number(s.total)<0||typeof s.running!=="boolean")return null;return{mode:s.mode,total:Number(s.total),running:s.running,startedAt:Number.isFinite(Number(s.startedAt))?Number(s.startedAt):0,elapsedBeforeStart:Math.max(0,Number(s.elapsedBeforeStart)||0),progressSessionId:typeof s.progressSessionId==="string"&&s.progressSessionId?s.progressSessionId:newProgressSessionId()}}catch{return null}}
...
export default function Timer(){
 ...
 const isProgressLeader=useCallback((now=Date.now())=>{
   const tabId=tabIdRef.current;if(!tabId)return false;
   try{
     const raw=localStorage.getItem(PROGRESS_LEADER_KEY);let current:any=null;
     if(raw){try{current=JSON.parse(raw)}catch{}}
     if(current&&current.tabId!==tabId&&Number(current.expiresAt)>now)return false;
     const next={tabId,expiresAt:now+PROGRESS_LEASE_MS};
     localStorage.setItem(PROGRESS_LEADER_KEY,JSON.stringify(next));
     const verify=JSON.parse(localStorage.getItem(PROGRESS_LEADER_KEY)||"null");
     return verify?.tabId===tabId&&Number(verify.expiresAt)>now;
   }catch{return false}
 },[]);
 ...
 const syncCloud=useCallback(async(timer:TimerState|null)=>{
   try{
     ...
     const activeTimer=timer?{run:timer.running,mode:timer.mode,total:timer.total,startedAt:timer.startedAt||Date.now(),elapsedBefore:timer.elapsedBeforeStart,progressSessionId:timer.progressSessionId}:null;
 ...
 useEffect(()=>{
   ...
   if(local){
     const now=Date.now();const elapsed=elapsedOf(local,now);progressSessionRef.current=local.progressSessionId||newProgressSessionId();
     if(local.running&&local.mode!=="Stopwatch"&&elapsed>=local.total){
       ...
       removeTimer();const reset={...local,running:false,startedAt:local.startedAt,elapsedBeforeStart:local.total};setState(reset);setLeft(0);void syncCloud(null);
     }else{setState(local);setLeft(remainingOf(local,now));lastProgressElapsedRef.current=elapsed;}
     setHydrated(true);return;
   }
   const loadCloud=async()=>{
     try{
       const auth=await fetch("/api/auth",{cache:"no-store"}).then(r=>r.json());
       if(auth.loggedIn){const r=await fetch("/api/sync",{cache:"no-store"});const x=await r.json();if(Number.isFinite(Number(x?.timerVersion)))cloudVersionRef.current=Number(x.timerVersion);const t=x?.data?.activeTimer;if(t?.run){const restored:TimerState={mode:modes.includes(t.mode)?t.mode:"Timer",total:Number(t.total)||DEFAULT_TOTAL,running:true,startedAt:Number(t.startedAt)||Date.now(),elapsedBeforeStart:Math.max(0,Number(t.elapsedBefore)||0),progressSessionId:typeof t.progressSessionId==="string"&&t.progressSessionId?t.progressSessionId:newProgressSessionId()};progressSessionRef.current=restored.progressSessionId||"";writeTimer(restored);setState(restored);setLeft(remainingOf(restored));lastProgressElapsedRef.current=restored.elapsedBeforeStart;}}
     }catch{}
     setHydrated(true);
   };
   void loadCloud();
 },[isProgressLeader,syncCloud]);
 ...
 const applyCustom=useCallback(()=>{const n=Math.min(20*3600,(+custom.h||0)*3600+(+custom.m||0)*60+(+custom.s||0));if(!n)return;if(stateRef.current.running)pauseTimer();const next={mode:"Timer" as Mode,total:n,running:false,startedAt:0,elapsedBeforeStart:0,progressSessionId:newProgressSessionId()};writeTimer(next);stateRef.current=next;setState(next);setLeft(n);lastProgressElapsedRef.current=0;progressSessionRef.current=next.progressSessionId||""},[custom,pauseTimer]);
